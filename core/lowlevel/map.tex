...
\cs_new_protected:Npn \__rpg_renew_map_item:
{
	...
	\ProvideDocumentCommand{\mapitem}{o}
	{
		...
		\tl_set:Ne \l__rpg_map_element_name_tl {\l__rpg_map_parent_name\RpgMapLevelName{\arabic{RpgAreaDepth}}{\l__map_tmp}}
		...
		}
		{
			...
			\bool_if:NTF\l__use_label
			{
				\tl_set:Ne \l__label_name {\l__label}
			}
			{
				\tl_set:Ne \l__label_name {\l__rpg_map_ref_tl##1}
			}
			%%spoof in the hidden tex!
			\tl_set:Ne \@currentlabelname{##1}
			\refstepcounter{RpgAreaDoubleCounter}
			\label{rpg__expanded:\l__label_name}
	...
	\ProvideDocumentCommand{\RpgArea}{o m}
	{
		\IfNoValueF{##1}
		{
