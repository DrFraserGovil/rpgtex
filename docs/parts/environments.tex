...
\labelsection{RpgCards}\label{S:Card}
    ...
    We anticipate that users won't access RpgCard directly, but will instead access it through wrappers which utilise the RpgSwitchEnv protocol \RpgPage[p]{RpgSwitchEnv} to allow items to be typeset `normally', and in card-mode. The main power of the RpgCard is the ability to automatically \textit{cardbreak}, splitting large internal elements across multiple cards. 

    \RpgMacro*{RpgCard}{\param{\paramO}}{Splits the contents across a number of playing-card sized units}{env-card
        }{
    ...
    \newcommand\widthLines[1]{
        %width and hmargin
        \draw[#1,<->] (0,2.4)-- node[below]{width}++(6.75,0);
        \draw[#1,<->] (0,2)-- node[below, rotate=90, anchor=east]{hmargin}++(0.5,0);
        \draw[#1,<->] (6.25,2)-- node[below, rotate=90, anchor=east]{hmargin}++(0.5,0);
    }
    \newcommand\heightLines[1]{
        %height and vmargin
        \draw[#1,<->] (0.7,0)-- node [right, rotate=90, anchor=north]{height}++(0,9.7cm);
        \draw[#1,<->] (1.4,0)-- node [right]{vmargin}++(0,0.5cm);
        \draw[#1,<->] (1.4,9.2)-- node [right]{vmargin}++(0,0.5cm);
    }
    \RpgGetExample{env-card-dimensions}
...
\labelsection{RpgMap}
    ...
    \RpgMacro[RpgMap!area]{area}{area, \param{\paramO \paramO}}
        ...
        { env-map-area}{
            ...
            \subsection{As an Environment}
                ...
                It is possible to nest maps within an \cmd{area} by first adding an area, and then creating a map. We also provide an interface to do this automatically, if \texttt{area} is called as an environment.
    ...
    \RpgGetExample{env-secret-hide}
    \RpgGetExample{env-secret-show}


\newpage
\labelsection{RpgTable}
    RpgTable is a wrapper for the \forcelink{https://ctan.org/pkg/tabularx}{tabularx} (or \forcelink{https://ctan.org/pkg/xltabular}{xltabular} or tabular-- see \texttt{breakable} and \texttt{simple}) environment, designed to provide attractive environments for the mainstay of RPG books since time immemorial: tables.

    \RpgGetExample{env-table-standard}
        

    \RpgMacro*{RpgTable}{\param{o m}}{Begins an environment for creating visually appealing and consistent tables. }{
        env-table-basic
    }{
        In default mode, RpgTable accepts the standard set of column specifications:\{c,l,r,p{width},\@,\ldots{}\} and the extended tabularx set (i.e. X). It therefore acts almost identically to the standard tabular environment with a few stylistic differences.
        ...
        \begin{description}
            ...
            \item[vskip] The vertical spacing above and below the table. Default value is \texttt{9pt plus 3pt minus 3pt}. 
            \item[filigree] If present, wraps the table in an \envref{RpgFiligree} environment, producing ornamental tables 
        \end{description}
    }
   
    
\labelsection{RpgNarration, RpgSidebar and RpgTip}

   

    \rpgtex{} defines three `colorbox' environments, which inherit from \tcref{}: \cmd{RpgSidebar}, \cmd{RpgNarration} and \cmd{RpgTip}. These provide a consistent way for a writer to highlight and differentiate blocks of text on the page.


    \subsection{Which colorbox to use?}
        The choice between these boxes (RpgSidebar and RpgTip especially) can be somewhat arbitrary -- any mechanical differentiation between them can be overriden by calls to the tcbox backend. Instead, the intention is that they serve slightly different purposes:
        ...
        \begin{description}
            ...
            \item[RpgNarration] is used for `read aloud' sections, such as those found at the beginning of adventure modules, or descriptions of environments.  
        \end{description}
    \RpgGetExample{env-text-boxes}
...
\section{Filigree}
    ...
    In design, Filigree is a decorative ribbon or other ornament around the outside of an element. We provide both a standard `RpgFiligree' environment and a tcolorbox style which implements the filigree.

    \RpgMacro*{rpgfiligree}{}{A \tcref{} style command that replaces the existing frame with a pre-defined overlay frame. }{
        env-filigree-cmd
    }{This command can be added to the options of any tcolorbox environment, and it will add the current filigree as the frame.}

    \RpgMacro*{RpgFiligree}{\param{o}}{A 'default' tcolorbox with the \texttt{rpgfiligree} applied to the ourside.}
    {
        env-filigree-env
    }
    {
        This environment is equivalent to calling \envref{RpgTip} with the filigree option enabled. See that documentation for customisation options.
    }

    \RpgGetExample{env-filigree-example}
\section{RpgItem, RpgSpell, RpgFeat: The FeatureForge}
    ...
    By default, all of these environments have the same behaviour.
    ...
    \RpgGetExample{env-full-featureforge}