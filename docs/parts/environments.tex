...
\labelsection{RpgMap}
    ...
    \RpgMacro{RpgArea}{RpgArea, \param{{o m}}}{
            ...
            ~
             ...
             \cmd{begin}\param{RpgArea}[manual-label]\param{<area name>}

            ~~~\ldots <nested-contents>

            \cmd{end}\param{RpgArea}
        }{
            \subsubsection{As a Command}
            ...
            \subsubsection{As an Environment}

             When called as an environment, the body-contents of the environment are wrapping the environment body in a nested \cmd{begin\param{RpgMap}} environment. This makes using RpgMap as an environment functionally equivalent to the following code:

             \begin{minipage}[t]{0.48\linewidth}
             \begin{Code}
                 \cmd{begin}\param{RpgArea}[label] \param{<area-name>}
            ...
            \end{minipage}\hspace{0.04\linewidth}\begin{minipage}[t]{0.48\linewidth}
             \begin{Code}
                 \cmd{RpgArea}[label]\param{<area-name>}
 ...
 \begin{figure*}[!t]
 \begin{ExampleBlock}[RpgMap,RpgArea,RpgArea]{Example RpgMap}
 \begin{RpgMap*}[
  header-offset=1,%start at subsection
  title={Example Map}]
  \RpgArea{The Spooky Mansion} 
     In this room, we can manually created a nested `RpgMap':
  \begin{RpgMap}[
        title={ignored-as-nested}]
     \RpgArea{Entrance Hall}
     \RpgArea{} An unnamed room
  \end{RpgMap}
  \begin{RpgArea}{The Creepy Grounds}
     The Grounds are an environment rather than a command, so these will be nested:
     \RpgArea{The gardens}
     \begin{RpgArea}{The stables}
         Areas can be artbirarily nested.
         \RpgArea{Shire horse}
  ...
  \RpgArea{The Graveyard}
    \begin{RpgMap}[
      blank-prefix={Tomb~}] %change prefix
      \RpgArea{}
        This is an unnamed area - it gets given a slightly different name
      \RpgArea[tomb-ref]{}
        We can refer to this later, even though it is unnamed.
     \end{RpgMap}
 \end{RpgMap*}
 \end{ExampleBlock}
 \end{figure*}
    ...
    \subsection{Map Labelling \& Referencing}
        ...
        \RpgMacro[RpgMap!Ref]{RpgMapRef}{\cmd{RpgMapRef*},\param{{m}}}{Returns the full name of the referenced area, including the map counter. The starred version returns only the map counter.}
            {
