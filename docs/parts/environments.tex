...
\section{RpgItem, RpgSpell, RpgFeat (etc.)}
    ...
    The prior documented environments have almost entirely been \textit{rules agnostic}. That is, they have primarily been typesetting aids which could equally be used for notes on a whimsical high fantasy game, or a gritty futuristic scifi game -- all the theme did was change the appearance of the individual elements. 
    ...
    \RpgMacro*{Rpg[X]}{\param{\paramO{} m \paramO}}{A card-switching environment which renders the text either in normal text, or as an \envref{RpgCard} }
    {
    ...
    \RpgMacro{getkey}{\param{m}}{Inserts the value passed to \texttt{key} in the \texttt{cmd-opts} args of the current [X]-environment}
        ...
        }
        {
            ...
            Note that this is \textit{not} a general feature of expl3-keys, and is an additional feature which is temporarily defined during the body of the environment. It is only able to retrieve key values from those added into the Parameter Registry \RpgPage[p]{S:ParameterRegistry}. 
        }
    
    
        
        \begin{ExampleBlock}{Feature Forge Example}
    \RpgMakeFeature{TestObject}{TestCard}{test}
    \ExplSyntaxOn
    \TestObjectAddProperty{test-key}{\l__test_key}{default}
            |\ExplSyntaxOff
    ...
    \begin{TestObject}{An Example}[test-key=virtue]
        This is the body text, I can see that my key is `\getkey{test-key}'
    \end{TestObject}
        \end{ExampleBlock}
