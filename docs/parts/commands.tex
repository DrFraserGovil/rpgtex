...
\section{Title \& Part Pages}
	...
	\RpgMacro{maketitle}{\param{}}
		{	
			...
			\cmd{maketitle} has been completely redefined, so commands such as \texttt{titlepage} won't work as expected. Instead, the appearance of the title page (or title header) are set by the theme, or user calls to either \cmdref{RpgSetCover} or \cmdref{RpgSetSimpleTitle}. Which of the two title `modes' is active is controlled by \cmdref{RpgSetCoverMode}.

			If `cover-mode' has been set, then the image stored in \texttt{\bs{}{@}cover} (if there is one) is automatically used as a full-page background image. This is independent of the drawing commands, and occurs before that function is called -- all subsequent drawing occurs over the top of the cover image.
		}
...
\section{Fonts \& Decorative Text}
	...
	\RpgMacro{emph}{\param{m},\cmd{key},\param{m}}{Uses the \texttt{RpgFontEmphasis} font to emphasise text.\index{key}}
		...
		{
			The \cmd{emph} command is usually a `context aware' emphasis command: equal to \cmd{textit} normally, \cmd{textbf} when the surrounding text is italics etc. However, for RPGs, it is convenient to be able to identify {keywords} in a consistent fashion. The \cmd{emph} command has therefore been redefined to use the \texttt{RpgFontEmphasis} font which can be configured to give a desired `keyword formatting'.
			
			The command \cmd{key} has also been provided, as a direct alias for \cmd{emph}.
		}
	\RpgMacro{RpgContour}{\param{\paramO{} m}}
		{	
			Renders text with a \RpgContour[inner=red,outer=black]{contour effect}. 
		}
		{
			\bs{}RpgContour[key/vals]\param{<text>}
		}
		{
            The optional argument accepts the following key/value pairs which alter the appearance of the contouring:
            \begin{RpgTable}{llX}
                Key & Argument Type & Effect
                \\
                inner & tikz color & Sets the color of the inner text
                \\
                outer & tikz color & Sets the color of the outer contour around the outside
                \\
                style & code & A style command (such as \cmd{bfseries}) which is applied to the text before contouring begins.
            \end{RpgTable}
			If the colors are not set, the default values are the \texttt{contourinnercolor} and \texttt{contouroutercolor} values defined by the theme \RpgPage[p]{S:Colors}.
			
			The contour does not automatically linebreak, but can be controlled manually with a \cmd{newline} command (not \cmd{\textbackslash} or \texttt{\textbackslash{}par})
	...
	\RpgMacro{RpgDropCap}{\param{\paramO{} m m}}
		...
		{
				\bs{}RpgDropCap[<lettrine-args>]\param{<letter>}\param{<text>}
		}{
...
\section{Theme Commands}
    ...
    \RpgMacro[RpgSet!Theme]{RpgSetTheme}{\param{{m}}}
        {
    ...
    \RpgMacro[RpgSet!Theme!Path]{RpgSetThemePath}{\param{{m}}}
        {
...
\section{Utility Commands}
 	 \RpgMacro{RpgDice}{\param{{m}}}
        ...
        {
            \bs{}RpgDice\param{<dice-expression>}
        }{
            ...
            \cmd{RpgDice} is neat, but not necessarily impressive by itself. The true power of the expression is that it calls the control sequence set by \cmdref{RpgDiceFormat} to perform the output formatting (after performing the regular expression parsing), allowing designers to customise their dice formatting.
        }
    ...
    \RpgMacro{RpgOrdinal}{\param{{o m}}}
        ...
        }
        {
            ...
            \begin{RpgTable}{XX}
                ...
                \tabverbExample{\RpgOrdinal[\textsuperscript]{13}}
    ...
    \RpgMacro{RpgPlural}{\param{{s o m m}}}
        {
        ...
        {
            The command outputs the count followed by the value of \texttt{<text>}. For a count of 1, the command then finishes. For any other count, it appends an ``s'', pluralizing the text. The starred version of the command suppresses the number output; so only the potentially-plural word is printed.
