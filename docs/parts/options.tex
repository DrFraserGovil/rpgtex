...
\section{Package Options}\label{S:PackageOptions}
	...
	\begin{Code}
		\bs{documentclass}[<options>]\param{rpgbook/rpghandout}

		\bs{usepackage}[<options>]\param{rpgtex}
	\end{Code}
	...
	\RpgMacro*{oneside}{}{Forces the document into oneside mode. Disables alternating footers but allows consistent asymmetric margins (such as in this document, where the left margin is larger to accommodate macro names)}{}{}
	\RpgMacro*{size}{}{Sets the font size equal to the value, if allowed by the parent class.}
		{
			size = <font-size>pt
		}
		{
			The allowed values depend on the class being used (see the class section, beginning on \RpgPage{S:bookClass}).
		}
...
\section{Colo(u)rs}\label{S:Colors}
	...
	\begin{description}
		...
		\begin{enumerate}
			\item \textbf{sidebarcolor} The background color of the \envref{RpgSidebar} environment
			\item \textbf{tablecolor} The background color of every other row in an \envref{RpgTable} 
			\item \textbf{tipcolor} The background color of the \envref{RpgTip} environment 
		\end{enumerate} 
		\item[narrationcolor]  The background color of the \envref{RpgTip} environment 
		\item[contourinnercolor]  The default color of the inner text within a \cmdref{RpgContour} command 
		\item[contouroutercolor]  The default color of the external contour drawn around text within a \texttt{RpgContour} command.
		\item[filigreecolor]  The default color of the filigree used in a \envref{RpgFiligree} environment.
	\end{description}
	...
	\end{Code}
\section{Section Names}\label{S:SectionNames}

	Since many themes customise their appearance beyond the simple use of \texttt{mark}s (such as \texttt{chaptermark}), we have provided direct access to the current name of the chapter, section, subsection and subsection:

	\begin{RpgSidebar}{Layout Only}
		These commands rely on redefining common commands, and therefore are only loaded when in layout mode (i.e. by rpgbook). 
	\end{RpgSidebar}

	% \RpgMacro{RpgChapterName}{}{The name given to the last \cmd{chapter} or \cmd{chapter*} command, or set by calling \cmd{chaptermark}}{}{}
	% \RpgMacro{RpgSectionName}{}{The name given to last \cmd{section} or \cmd{section*} command, or set by calling \cmd{sectionmark}}{}{}
	% \RpgMacro{RpgSubsectionName}{}{The name given to the last \cmd{subsection} or \cmd{chapter*} command, or set by calling \cmd{sectionmark}}{}{}
	% \RpgMacro{RpgSubsubsectionName}{}{The name given to the last \cmd{subsubsection} or \cmd{subsubsection*} command, or set by calling \cmd{subsection}}{}{}

	\RpgMacro{RpgGetName}{\param{m}}{Returns the name given to the last section matching the input command}
	{
		\cmd{RpgGetName}{chapter/section/subsection/subsubsection}
	}
	{
		If the input value is the name of a vaild section, returns the previous name given to that kind of section.

		These names can also be `spoofed' by manually calling a \cmd{mark} (such as \cmd{chaptermark} etc.), and we also provide the \cmdref{RpgFakeChapter} command.
	}

\begin{ExampleBlock}{Section Names}
   We are in Chapter `\RpgGetName{chapter}', section `\RpgGetName{section}'.
   ...
   I believe I am in a \RpgGetName{chapter}, though no chapter was actually called. 
   
   \chaptermark{Options \& Variables} %%put it back to the real value!
...
\labelsection{Fonts}
	...
	\subsection{Font Elements}
		...
		\index{Fonts|see {RpgFont[X]}}\index{RpgFont[X]}
		\begin{fontlist}
			...
			\FontElement{RpgFontStatBlockTitle}{stat-block-title}{The font used for the title of a `statblock' environment - in the dnd theme this corresponds to the \texttt{monster} environment.}
			\FontElement{RpgFontStatBlockSection}{stat-block-section}{The font used for sections within a `statblock' environment (should one be defined).}
			\FontElement{RpgFontStatBlockBody}{stat-block-body}{The font used for text within a `statblock' environment (should one be defined).}
