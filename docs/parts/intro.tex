...
ln -sf ~/your/rpgtex/directory "$(kpsewhich -var-value TEXMFHOME)/tex/latex/rpgtex"
		...
		\subsection{Overleaf (Not recommended!)}
			...
			\begin{enumerate}
				...
				\item Manually create the file \texttt{rpg-config.cfg} with the contents ``\cmd{edef}\cmd{RpgPackagePath}\param{../}''. This replaces the configuration step described below.
			\end{enumerate}
	...
	\section{Configuring \rpgtex{}}\label{S:Configuration}
		...
		Because \rpgtex{} comes packaged with some resources (namely some fonts) for which \TeX{} does not support relative-pathfinding, it is necessary to provide \rpgtex{} with a value for the \cmd{RpgPackagePath}.
		
		\RpgMacro{RpgPackagePath}{}{A variable holding the absolute path to the root directory of the rpgtex install location.}{}{
			We provide a number of different ways to set this variable:
			\begin{enumerate}
				\item The provided \texttt{rpglatex} compiler automatically inserts a definition based on a \texttt{kpsewhich}, using the Command Line Interface \RpgPage[p]{S:CMD}.
				\item If the variable does not exist, the \rpgtex{} engine itself tries the following
				\begin{enumerate}
					\item Searches for the file \texttt{core/rpg-config.cfg}, which (hopefully) contains a definition\footnote{In the form of \cmd{edef}\cmd{RpgPackagePath}\param{path/to/package}}, and \cmd{inserts} it.
					\item If no file is found, \rpgtex{} used the \cmd{write18} utility to dispatch its own calls to \cmd{kpsewhich} and build a config file in the expected location. This requires the compiler be run with \texttt{--shell-escape} active
					\item If this doesn't work, the compiler exits, and suggests that the user manually creates the config file:
					\begin{Code}
						cd <rpgtex root directory>

						cmd="\cmd{edef}\cmd{RpgPackagePath}\param{\$(pwd)}"
