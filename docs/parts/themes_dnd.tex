...
\section{RpgStat}
    ...
    \RpgMacro*[RpgStat!Theme!dnd]{RpgStat}{\param{\paramO m \paramO}}{ The dnd configuration of the \envref{RpgStat} environment, used to describe monsters and enemies, using the rules and parameters of spells in D\&D 5e. As with all FeatureForge environments, RpgStat has the ability to switch between `text mode' and `card mode' depending on the value passed to \cmd{RpgStatShowCard} \RpgPage[p]{Macro:Rpg[X]ShowCard}. }
        ...
        \subsection{Statistics Options}
            ...
            \newcommand{\statkey}[4]{\\\parbox[t]{2cm}{\raggedright #1} & #2 & #3 & #4}
            \newcommand{\catbreak}[1]{\\\multicolumn{4}{l}{\textit{#1}}}
            \begin{RpgTable}[breakable]{lllX}
                Key & Type & Default Value &Effect 
                \catbreak{Appearance \& Visual Tweaks}
                    ...
                    \statkey{outline-color}{tikz-color code}{--empty--}{If non-empty, this color is used for the border of the environment, intstead of the one derived from 'color'}
                    \statkey{filigree}{tikz-color code}{--no value--}{}
                \catbreak{Appearance}
            \end{RpgTable}
