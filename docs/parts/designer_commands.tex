...
\RpgMacro[RpgDice!SetFormat]{RpgDiceSetFormat}{\param{{+m}}}
        ...
        }
        {
            ...
            \cmd{RpgDiceSetFormat\param{\#1d\#2 \#3}}, such that \cmd{RpgDice\param{ndx + a + b}} gives ``ndx + c'', where c is the numerical value of a+b, with an additional check to see if \texttt{\#3} is equal to 0 (to avoid `1d6 + 0'). 
...
\begin{ExampleBlock}{RpgDice Formatting: The D\&D Format}
 ...
 \RpgDiceSetFormat{ % #1: Dice Number, #2: Dice Sides, #3: Modifier
  %%Set dX -> 1dX
