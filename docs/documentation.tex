...
\documentclass[theme=default,justified,a4paper,oneside]{rpgbook}
...
\NewDocumentCommand{\lcaseidx}{m}{
	\lowercase{\def\temp{#1}}
	\expandafter\index\expandafter{\temp@\texttt{#1}}
}
\NewDocumentCommand{\cmdidx}{ o m }{
	\foreach \m in {#2}
	{
		...
		{
			 \lowercase{\def\temp{\m}}
			\expandafter\index\expandafter{\temp@\texttt{\m}}
		 }
		 {
			% \index{\m}
			 \lowercase{\def\temp{\m}}
			\expandafter\index\expandafter{#1!\temp@\texttt{\m}}
		 }
	}
}
