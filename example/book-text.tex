...
\part[engine_part]{Basic Commands}
	...
	\chapter{Sections}
		...
		% 	\RpgSetAreaOptions[area-label=village,area-num-header-offset=3]{}
		% 	\RpgArea{Village of Hommlet}
		% 	This is the village of hommlet.
		% 	\RpgSetAreaOptions[area-label=villages,area-num-header-offset=3]{}
		...
		% 		Map headers can be set to different `levels', using either section/subsection (etc.) for display purposes. The default value is \verb|\RpgSetAreaOptions[area-num-header-offset=2]{}| (subsection). For display purposes here, we used \verb|area-num-header-offset=3|
		\section{RpgTable}
...
\end{RpgSidebar}
		...
		\begin{RpgMap*}[header-offset=0,title={RPG maps}]
			...
			\begin{RpgNestedArea}[bob][prefix={Bob~}]{Bob's House}
				\RpgArea{Front Room} The TV is here, and it is on
				\RpgArea{Kitchen}
				\RpgArea[cot]{}
			\end{RpgNestedArea}

			\RpgArea{Karen's House} Karen lives here
			\RpgArea{}
		\end{RpgMap*}

		\begin{RpgMap}[header-offset=0,title={A test map}]
			
			\RpgArea{Bob's House} Bob lives here
		...
		Let's test: \RpgMapRef*{Map:Front Room}
